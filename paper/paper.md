...
revision. \autoref{fig:scaling} reports single-threaded NumPy results on an Intel
Core Ultra 5 225U using seven interleaved repeats. “Supplied sites” means sites
in the structure passed to both calculators, before primitive-cell reduction.
For P1 cells containing 4--256 irreducible sites, cached calculations are
8.5--35.5 times faster than pymatgen and complete end-to-end calculations are
7.9--13.7 times faster. For NaCl supercells containing 8--512 supplied sites
but reducing to two primitive sites, cached speedups rise from 67 to 9383. The
end-to-end calculation is 0.83 times as fast for the smallest cell because
symmetry preprocessing dominates, crosses over by 64 supplied sites, and
reaches 79.6 times at 512 sites. CUDA results are intentionally absent until
measurements from the same validated protocol are available; the benchmark
schema and plotting script accept an additional PyTorch/CUDA run without
changing the CPU data.

![Runtime and speedup for increasing irreducible P1 cells (left) and
symmetry-reducible NaCl supercells (right). Small points show the seven timing
repeats or their paired speedup ratios. Large runtime points are medians; large
speedup points are ratios of medians; error bars span the corresponding
interquartile ranges. Cached timings reuse symmetry and reflection topology;
end-to-end timings include all preprocessing.\label{fig:scaling}](figures/scaling_speedup.pdf){width="100%"}
