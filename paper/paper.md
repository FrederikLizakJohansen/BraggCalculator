...
workflows. `BraggCalculator` [@braggcalculator2026] is a Python package that
simulates ideal, monochromatic powder X-ray and neutron diffraction from
periodic crystals. It accepts CIF files and structures from pymatgen or the
Atomic Simulation Environment (ASE) [@larsen2017], and returns indexed
reflections, powder lines, or broadened profiles in scattering angle or
...
`BraggCalculator` addresses this need with a scriptable and verifiable forward
model for periodic crystals. When a structure is loaded, it determines a
primitive cell and prepares the reciprocal-lattice points that can contribute
...
refinement [@toby2013]. `BraggCalculator` is a focused forward calculator rather
than a replacement for a refinement suite. In the validation and performance
comparisons below, pymatgen serves as an implementation reference: the two
calculators evaluate the same kinematic model but organize the computation
differently. In the benchmarked versions, pymatgen constructs reciprocal points
and site arrays for each pattern and loops over reflections in Python, with the
site sum for each reflection vectorized in NumPy. `BraggCalculator` prepares
the primitive cell and reciprocal-lattice points once, evaluates all reflection--site
contributions in bounded vectorized chunks, and groups coincident powder lines
with indexed array reductions. Cached calls reuse the prepared data, and
symmetric supercells benefit because the numerical work follows the primitive
cell. These changes reorganize the calculation without omitting reflections or
site contributions; the comparisons below quantify the resulting numerical
agreement.

`BraggCalculator` was developed alongside `DebyeCalculator` [@johansen2024], a
GPU-accelerated package for powder and total scattering from finite,
disordered, and non-crystalline structures. Its real-space Debye pair sum
complements `BraggCalculator`'s reciprocal-space treatment of periodic
crystals.
...
unchanged; the calculation must be rebuilt if a reflection enters or leaves
the requested range. Phase matrices and profile kernels are evaluated in
bounded chunks, trading a small loop over chunks for predictable memory use on
...
\autoref{fig:oracle} cover NaCl, SrTiO$_3$, and a triclinic SiO$_2$ cell. Across
172 X-ray powder lines, the largest position difference is
...
pointwise BraggCalculator-minus-pymatgen difference on the displayed scientific
scale.
...
comparisons lie on equality. (c,d) Maximum line-position and
normalized-intensity errors expressed as orders of magnitude below their
...
(\autoref{fig:cod-throughput}).

![One-pass throughput on the frozen COD corpus after CIF parsing. (a) Summed
runtime for 70 X-ray and 70 neutron calculations; unhatched and hatched segments
denote the two modes, and the pymatgen bar uses the median per-mode totals from
the three matched records. (b) Same-record pymatgen-to-BraggCalculator ratios
for X-ray circles, neutron triangles, and the combined-total diamonds. (c)
Per-structure CUDA speedup over pymatgen. (d) Direct matched PyTorch CPU-to-CUDA
runtime ratio, where values above one favor CUDA. Points in (c,d) are single
distinct structure--radiation calculations; aggregate ratios use summed
runtimes across the full corpus.
\label{fig:cod-throughput}](figures/cod_throughput.pdf){width="100%"}
...
thread limits, and timing protocol. “Supplied sites” counts the sites in the
structure passed to both calculators, before primitive-cell reduction.
...
every case to the same two-site primitive cell and 410 reciprocal-lattice
points. The GPU workload therefore remains fixed as the supplied supercell
grows, leaving the CUDA calculation dominated by launch, transfer, and
synchronization overhead. Against the matched PyTorch CPU run, CUDA is
...
advantage. Cached timings reuse symmetry analysis and the prepared reflection
set; end-to-end timings include all preprocessing.
