...
single-threaded NumPy run on an Intel Core Ultra 5 225U and a float64 PyTorch
run on an NVIDIA RTX A3000 Laptop GPU under WSL2. “Supplied sites” means sites
...
grows, reaching cached and end-to-end speedups over its host pymatgen of 190
and 132 times, respectively, at 256 sites.
...
rather than throughput-bound. Its end-to-end speedup over host pymatgen rises
from 0.81 to 77.1 times, closely matching the NumPy run's 0.83 to 79.6 times;
the increasing advantage comes from symmetry reduction rather than additional
GPU work. Because the two records were collected on different host machines
and dependency versions, each plotted speedup uses pymatgen from the same
record. Absolute runtimes across the circle and square series are descriptive
multi-hardware measurements, not controlled CPU-versus-GPU acceleration
ratios.

![Runtime and speedup for increasing irreducible P1 cells (left) and
symmetry-reducible NaCl supercells (right). Circles denote the Intel CPU record
and squares the NVIDIA CUDA record. Small points show the seven timing repeats
or their paired speedup ratios. Large runtime points are medians; large speedup
points are ratios of medians; error bars span the corresponding interquartile
ranges. Cached timings reuse symmetry and reflection topology; end-to-end
timings include all preprocessing. Each speedup is relative to pymatgen on the
same host.\label{fig:scaling}](figures/scaling_speedup.pdf){width="100%"}
