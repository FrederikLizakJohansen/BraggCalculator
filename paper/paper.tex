...
\citep{johansen2024}, a complementary GPU-accelerated package that computes
powder and total scattering from atomic pair distances using the Debye
scattering equation. Together, the packages provide two routes from atom
coordinates to scattering for different structural regimes.
\texttt{DebyeCalculator} is suited to finite, disordered, and non-crystalline
structures, where retaining every atomic pair preserves finite-size and
non-periodic information, at the cost of a pair sum that scales quadratically
with atom count. \texttt{BraggCalculator} addresses periodic crystals instead:
it uses translational symmetry and evaluates the finite set of reciprocal-space
reflections in the requested range. Keeping the two engines separate reflects
this physical and computational division, while giving
\texttt{BraggCalculator} a focused scope distinct from both real-space
scattering codes and general refinement suites.
...
\begin{figure}[htbp]
  ...
  trace is their pointwise difference on the displayed scientific scale.}
  \label{fig:oracle}
\end{figure}

A frozen corpus test, summarized in Figure~\ref{fig:cif-validation}, broadens
this oracle comparison beyond constructed examples. Seventy CC0 CIF records
...
\begin{figure}[htbp]
  ...
  and normalized-intensity errors expressed as decimal orders below their
...
single WSL2 host with an NVIDIA RTX A3000 Laptop GPU. The NumPy CPU, PyTorch
CPU, and float64 PyTorch CUDA records use the same revision, dependencies,
cases, thread limits, and timing protocol.
...
\begin{figure}[htbp]
  ...
  above one indicate a CUDA advantage. Cached timings reuse symmetry and
  reflection topology; end-to-end timings include all preprocessing.}
  \label{fig:scaling}
\end{figure}
